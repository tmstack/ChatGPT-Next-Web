...
export class ChatGPTApi implements LLMApi {
  ...
  async chat(options: ChatOptions) {
    ...
    const isClaude = options.config.model.toLowerCase().startsWith("claude");
    if (isDalle3) {
      ...
      // Only add top_p for non-O1/O3 and non-Claude models
      if (!isO1OrO3 && !isClaude) {
        (requestPayload as RequestPayload).top_p = modelConfig.top_p;
      }
